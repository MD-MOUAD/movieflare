...
const CoverComponent = ({item}) => {
  ...
  return (
    <Link to={`/`}>
      <div className="relative bg-cover bg-center" style={{ backgroundImage: `url("${BaseImgPathOriginal}/${item.backdrop_path})")` }}>
      ...
      <div className="relative p-6 md:p-12 rounded-lg">
        <div className="flex items-center text-white w-fit max-w-7xl bg-opacity-75 sm:backdrop-filter sm:backdrop-blur-sm">
          <img src={`${BaseImgPathOriginal}/${item.poster_path}`} alt="Movie Poster" className="w-28 lg:w-48 rounded-lg shadow-lg opacity-65 max-sm:hidden" />
          <div className="mt-6 md:mt-0 ml-3 md:ml-6 text-left">
            <h1 className="text-2xl md:text-4xl lg:text-5xl font-bold text-cyan-400">{title}</h1>
            <p className="mt-2 md:mt-4 text-sm lg:text-lg max-sm:hidden">
              {item?.release_date && "Release" || "First air"} Date: <span className="font-semibold">{releaseDate}</span>
            </p>
            <p className="md:mt-2 text-sm lg:text-lg max-sm:hidden">
              Genre: <span className="font-semibold">Action, Adventure</span>
            </p>
            <p className="mt-4 text-base line-clamp-2 max-md:hidden">
              {item?.overview}
            </p>
            <div className="mt-6">
