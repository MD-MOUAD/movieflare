...
const CoverComponent = ({item}) => {
  return (
    <Link to={`/`}>
      {/* <div 
        className="w-full h-[45vh] bg-cover bg-center bg-no-repeat -z-10 opacity-5 relative"
        style={{ backgroundImage: `url("${BaseImgPathOriginal}/${item.backdrop_path})"`}}>
          <img src={`${baseImgPath}/${item.poster_path})`} className="h-1/2" />
      </div> */}
      <div className="relative h-[45vh] flex items-center justify-center bg-cover bg-center" style={{ backgroundImage: `url("${BaseImgPathOriginal}/${item.backdrop_path})")` }}>
