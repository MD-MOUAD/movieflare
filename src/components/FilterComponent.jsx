...
const FilterComponent = ({ mediaType, setGenre, setSort, resetPage }) => {
  ...
  const sortOptions = [
    { value: "popularity.desc&vote_count.gte=100", label: "Most Popular" },
    { value: "vote_average.desc&vote_count.gte=1000", label: "Top Rated" },
    {
      value: `${titleOptionValue}.asc&vote_count.gte=100`,
      label: `${titleOptionLabel} (A-Z)`,
    },
    {
      value: `${titleOptionValue}.desc&vote_count.gte=100`,
      label: `${titleOptionLabel} (Z-A)`,
    },
    { value: "popularity.asc&vote_count.gte=100", label: "Least Popular" },
  ];
    ...
    <div className="filter hidden px-2 py-4 rounded-md bg-slate-300 dark:bg-neutral-900">
      <form className="flex flex-wrap items-center gap-10 max-lg:gap-2">
        <Select
          ...
          placeholder='Contain genres "Any"'
